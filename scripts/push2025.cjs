/**
 * push2025.cjs — Push 2025 import data into Firebase week by week.
 *
 * Usage:
 *   node scripts/push2025.cjs          # load all weeks
 *   node scripts/push2025.cjs 1        # load only week 1
 *   node scripts/push2025.cjs 1 5      # load weeks 1 through 5
 *   node scripts/push2025.cjs --reset  # wipe league-2025 and start fresh
 *
...
const DOC = db.collection("pvgc").doc("league-2025");

const DATA_FILE = path.join(__dirname, "..", "data", "2025-import.json");
...
async function reset() {
  console.log("Resetting league-2025 …");
  await DOC.set({
    handicaps: data.defaultHcp,
    results: {},
    hcpOverrides: {},
...
(async () => {
  try {
    if (args[0] === "--reset") {
      ...
      const to   = args[1] ? parseInt(args[1]) : 21;
